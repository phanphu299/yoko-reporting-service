...
{
    public class ReportExportHandler : BaseExportHandler<ReportDto>, IExportHandler
    {
        private const string MODEL_NAME = "Report";
        ...
        public ReportExportHandler(IConfiguration configuration, ITenantContext tenantContext,
            ...
            try
            {
                ...
                query = @"select
                        ...
                        from reports as r with(nolock)
                            join storages s with(nolock) on s.id = r.storage_id
                ...
                {
                    reports = await dbConnection.QueryAsync<ReportDto>(query, new { Ids = request.Ids }, commandTimeout: 600);
        ...
        private async Task CreateMultipleFileContentAsync(IEnumerable<ReportDto> reports, Stream contentStream)
        {
            using (var zipArchive = new ZipArchive(contentStream, ZipArchiveMode.Create, true))
            {
                ...
                {
                    var newEntry = zipArchive.CreateEntry(r.FileName);
